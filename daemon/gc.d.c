...
void child( struct KERNEL *kernel, uint64_t pid ) {
	...
	for( uint64_t i = 0; i < KERNEL_TASK_limit; i++ ) {
		...
		if( kernel -> task_base_address[ i ].flags && kernel -> task_base_address[ i ].pid_parent == pid ) {
			// close all child processes belonging to this parent
			child( kernel, kernel -> task_base_address[ i ].pid );
...
void entry( struct KERNEL *kernel ) {
	...
	while( TRUE ) {
		...
		for( uint64_t i = 0; i < KERNEL_TASK_limit; i++ ) {
			// task closed?
			if( kernel -> task_base_address[ i ].flags & KERNEL_TASK_FLAG_close ) {
				...
				if( kernel -> task_base_address[ i ].flags & KERNEL_TASK_FLAG_thread ) {
					// remove redundant entries in threads paging table
					uint64_t *pml4 = (uint64_t *) (kernel -> task_base_address[ i ].cr3 | KERNEL_PAGE_mirror);
					for( uint16_t p = 0; p < KERNEL_PAGE_SIZE_entry; p++ )
						// redundant entry?
						if( p != KERNEL_PAGE_ENTRY_stack && p != KERNEL_PAGE_ENTRY_stack_context ) pml4[ p ] = EMPTY;
				} else
					// release memory map
					kernel -> memory_release( (uintptr_t) kernel -> task_base_address[ i ].memory_map, MACRO_PAGE_ALIGN_UP( kernel -> page_limit >> STATIC_DIVIDE_BY_8_shift ) >> STATIC_PAGE_SIZE_shift );

				// release paging structure and page itself
				kernel -> page_deconstruction( (uintptr_t *) kernel -> task_base_address[ i ].cr3 );

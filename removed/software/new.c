...
vector3f move = { 0.0f, 0.0f, 16.0f, 1.0f };
...
int64_t _main( uint64_t argc, uint8_t *argv[] ) { init( argc, argv );
	...
	while( TRUE ) {
		...
		while( rotate_x > 360.0f ) rotate_x -= 360.0f;
		while( rotate_y > 360.0f ) rotate_y -= 360.0f;
		while( rotate_x < 360.0f ) rotate_x += 360.0f;
		while( rotate_y < 360.0f ) rotate_y += 360.0f;
...
// vector3f upN = lib_rgl_multiply_vector( upC, matRXY );
		...
		lib_rgl_flush();
		interface.window_descriptor -> flags |= WM_DESCRIPTOR_FLAG_flush | WM_DESCRIPTOR_FLAG_cursor;
